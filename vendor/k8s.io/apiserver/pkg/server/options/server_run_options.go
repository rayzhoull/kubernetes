...
type ServerRunOptions struct {
	AdvertiseAddress net.IP		//服务绑定的IP和端口

	CorsAllowedOriginList       []string	//允许跨域的域名列表
	ExternalHost                string	//主机名
	MaxRequestsInFlight         int	//最大连接数
	MaxMutatingRequestsInFlight int	//变更请求最大连接数
	RequestTimeout              time.Duration	//请求超时时间
	GoawayChance                float64	//随机关闭一个连接，使其再次负载均衡
	LivezGracePeriod            time.Duration //启动序列所需最大时间。
	MinRequestTimeout           int	//最小超时时间，watch可根据此选择一个大于此的数字作为超时时间
	ShutdownDelayDuration       time.Duration //延时关闭服务时间
	...
	// We intentionally did not add a flag for this option. Users of the
	// apiserver library can wire it to a flag.
	MaxRequestBodyBytes       int64	//request请求体最大字节数
	TargetRAMMB               int	//内存限制
	EnablePriorityAndFairness bool	//API优先级
}

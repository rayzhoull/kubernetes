...
type EtcdOptions struct {
	...
	EncryptionProviderConfigFilepath string	//用于加密的配置文件路径

	EtcdServersOverrides []string	//etcd服务器列表 格式：group/resource#servers

	// To enable protobuf as storage format, it is enough
	// to set it to "application/vnd.kubernetes.protobuf".
	DefaultStorageMediaType string 		//默认存储类型
	DeleteCollectionWorkers int	//为DeleteCollection调用派生的worker数量。它们用于加速名称空间清理。
	EnableGarbageCollection bool	//启用通用垃圾收集器。必须与kube-controller-manager的对应标志同步。

	// Set EnableWatchCache to false to disable all watch caches
	EnableWatchCache bool  //启用监视缓存
	// Set DefaultWatchCacheSize to zero to disable watch caches for those resources that have no explicit cache size set
	DefaultWatchCacheSize int //缓存容量大小监视
	// WatchCacheSizes represents override to a given resource
	/*
	观察一些资源(pods, nodes)、逗号分隔的缓存大小设置。个别设置格式:resource[。group]#size，其中资源是小写复数(没有版本)，
	对于apiVersion v1(遗留的核心API)的资源，group被省略，而对于其他的资源，group被包含，size是一个数字。它在启用监视缓存时生效。
	一些资源(复制控制器、端点、节点、pods、服务、apiservice .apiregistration.k8 .io)有由启发式设置的系统默认值，其他资源默认为默认监视缓存大小。
	 */
